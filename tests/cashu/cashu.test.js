...
function expectNUT10SecretDataToEqual (p, s) {
  p.forEach(p => {
...
describe('mint api', () => {
  ...
  test('send and receive p2pk', async () => {
    ...
    expect(
      proofs.reduce((curr, acc) => curr + acc.amount, 0)
    ).toBe(63)
  })
...
describe('dleq', () => {
  ...
  test('send not enough proofs when dleq is required', async () => {
    ...
    // strip dleq
    proofs = proofs.map(p => ({...p, dleq: undefined}))
